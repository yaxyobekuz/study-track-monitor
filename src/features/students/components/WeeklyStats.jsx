...
// API
import { statisticsAPI } from "@/features/students/api/statistics.api";

const WeeklyStats = ({ studentId }) => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["monitor", "statistics", "student-weekly", studentId],
    queryFn: () =>
      statisticsAPI
        ...
        items={[
          {
            ...
            trailing: (
              <p className="font-semibold text-cyan-600 xs:text-lg sm:font-bold sm:text-xl md:text-2xl">
                <Counter value={rankings?.schoolRank ?? 0} /> /{" "}
                <Counter value={rankings?.schoolTotalStudents ?? 0} />
              </p>
            ),
          },
